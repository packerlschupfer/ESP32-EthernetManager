...
void setup() {
    ...
    Serial.println();
    Serial.println("==============================");
    Serial.println("  ESP32 Ethernet Project");
    Serial.println("==============================");
...
void loop() {
    ...
    // Feed the watchdog for loopTask. A failed feed means this task is no
    // longer registered, which will eventually trigger a watchdog reset.
    if (!taskManager.feedWatchdog()) {
        LOG_WARN(LOG_TAG_MAIN, "Watchdog feed failed for loopTask");
    }
...
void handleShutdown() {
    ...
    if (monitoringHandle != nullptr) {
        if (!taskManager.stopTask(monitoringHandle)) {
            LOG_WARN(LOG_TAG_MAIN, "Failed to stop MonitoringTask");
        }
    }
    
    // Unregister main loop from watchdog
    LOG_INFO(LOG_TAG_MAIN, "Unregistering from watchdog...");
    if (!taskManager.unregisterCurrentTaskFromWatchdog()) {
        LOG_WARN(LOG_TAG_MAIN, "Failed to unregister from watchdog");
    }
